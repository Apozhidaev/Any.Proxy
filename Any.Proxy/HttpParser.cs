...
namespace Any.Proxy
{
    public class HttpParser
    {
        ...
        private readonly byte[] _source = null;
        ...
        public HttpParser(byte[] source)
        {
            ...
            var regMethod = new Regex(@"(?<method>.+)\s+(?<path>.+)\s+HTTP/(?<version>[\d\.]+)", RegexOptions.Multiline | RegexOptions.IgnoreCase);
            var matchMethod = regMethod.Match(httpInfo);
            ...
            var regHost = new Regex(@"Host: (((?<host>.+?):(?<port>\d+?))|(?<host>.+?))\s+", RegexOptions.Multiline | RegexOptions.IgnoreCase);
            var matchHost = regHost.Match(sourceString);
            ...
            var regHeaders = new Regex(@"^(?<key>[^\x3A]+)\:\s{1}(?<value>.+)$", RegexOptions.Multiline | RegexOptions.IgnoreCase);
